...
rest_init(Req, _Opts) ->
    ...
    {Origin1, Req3} = cowboy_req:header(<<"origin">>, Req2, <<"*">>),
    {Origin, Req4} = cowboy_req:header(<<"headers">>, Req3, <<"*">>),
    Req5 = cowboy_req:set_resp_header(<<"Access-Control-Allow-Origin">>, Origin1, Req4),
    Req6 = cowboy_req:set_resp_header(<<"Access-Control-Allow-Headers">>, Origin, Req5),
    {ok, Req6, #st{resource_module = Module, resource_id = Id}}.
...
allowed_methods(Req, #st{resource_id = undefined} = State) -> {[<<"GET">>, <<"POST">>, <<"OPTIONS">>], Req, State};
allowed_methods(Req, State)                                -> {[<<"GET">>, <<"PUT">>, <<"DELETE">>, <<"OPTIONS">>], Req, State}.
